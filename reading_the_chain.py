...
def connect_to_eth():
	url = f"https://mainnet.infura.io/v3/41831f4982e74499b13d43b443f36666"
...
def connect_with_middleware(contract_json = "contract_info.json"):
	...
	url = f"https://bsc-testnet.publicnode.com"
	w3 = Web3(HTTPProvider(url))
	assert w3.is_connected(), f"Failed to connect to provider at {url}"
...
def is_ordered_block(w3, block_num):
	...
	tx_transactions = block.transactions
  
	if len(tx_transactions) <= 1:
		return True
	...
	def check(tx):
		...
		if max_fee is not None and max_priority is not None:
			return min(int(max_priority), int(max_fee) - int(base_fee))
		else:
			return int(gas_price) - int(base_fee)

	ordered_fee = None
	for tx in tx_transactions:
		fee = check(tx)
		if ordered_fee is not None and fee > ordered_fee:
			ordered = False
			break
		ordered_fee = fee
...
if __name__ == "__main__":
	...
	print("contract values", get_contract_values(contract, admin_address, owner_address))
